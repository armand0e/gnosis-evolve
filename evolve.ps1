...
function Start-ClaudeExecutable {
    param (
    ...
    if ($ExecutablePath) {
        $actionVerb = if ((Get-Process -Name "*claude*" -ErrorAction SilentlyContinue)) { "Restarting" } else { "Starting" }
        Write-Host "$actionVerb Claude from: $ExecutablePath" -ForegroundColor Green
    ...
    if ($claudeProcesses) {
        ...
        if ($Kill -or $Restart) {
            ...
            if ($Restart) {
                ...
                return Start-ClaudeExecutable -ExecutablePath $claudeExe -WaitTime $WaitTime
            }
        }
    } else {
